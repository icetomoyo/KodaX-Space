...
> **2026-09-08 当前源码候选**：Space package 为 `0.1.46-alpha.8`，root/Desktop/lockfile 已精确锁定已发布的
> `@kodax-ai/kodax@0.7.96-beta.3`。SDK 包门要求 `sandboxRuntime:11`、`runtimeAutoModeGuardrail:5`、`sharedSessionSettings:2` 与 `effectiveConfig:1`；daemon 准入与连接后 Runtime 门额外要求 `providerCredentialBroker:2`。Windows native protocol/setup generation 10、真实 target-start doctor 证明、私有逐命令 Temp 与 64 端口 broker 范围都由 v11 门隔离。KodaX
...
> alpha.8 发布范围与验证门见 [发布准备记录](releases/v0.1.46-alpha.8-release-readiness.md)，压缩与沙箱回归步骤见 [Issue 210 指南](test-guides/ISSUE_210_v0.1.46_REGRESSION_GUIDE.md)。
