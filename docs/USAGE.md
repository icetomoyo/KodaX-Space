...
>
> 当前 Space 包/发布准备基线：`v0.1.32`；SDK 基线为 npm Registry 正式发布的精确 `@kodax-ai/kodax@0.7.74`。
...
根、desktop manifest 与 lockfile 都固定到精确 KodaX 0.7.74。官方包 URL 为 `https://registry.npmjs.org/@kodax-ai/kodax/-/kodax-0.7.74.tgz`，SRI 为 `sha512-JgPE6ct9m5l2e9F5dQIYnNdoKaMIJF1gWnyUzUowAWFva4JksE0DScANNUHEdQYjoibXwBlcNgOIse5bAfRvGg==`，SHA256 为 `61D66EA31599A5FBFEA8E5779A4BC238933A1DC000005A04757DD69A1F98F2C6`。`npm ls @kodax-ai/kodax --all` 应只显示同一个 deduped 版本；Runtime compatibility 会严格要求 guardrail v3、`permission:grant-admin`、`interruptInput:1`、`actorControlPlane:1`、`contextCompaction:3`、`transcriptPaging:1` 和 `transcriptSearch:1`，并直接验证 PowerShell 方括号路径边界、非空 auto-resume 导出和跨进程共享。干净安装应使用 `npm ci`，不得改回同版本的本地 tgz。
