# Issue 181 — Provider Recovery Duplicate Output Regression Guide (v0.1.41)

## Automated gates

1. Run the projection and renderer regressions:
...
does not apply events beyond the observation cursor or discard a retained stable
prefix.
...
3. Repeat with `non_streaming_fallback` and thinking-content sanitization.
...
6. Send a prompt that intentionally produces repeated identical text without a
   Provider recovery. Confirm that legitimate repetition is preserved.
