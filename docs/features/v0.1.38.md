...
> Status: Planned
> Feature: F129
> Roadmap rebase: this release replaces the former `v0.1.38` stabilization reserve with a dedicated Partner presentation release. `v0.1.33` remains the near-term stabilization lane, `v0.1.34` carries the deferred F137/F139 feature lane, and `v0.1.44` remains the final 0.1.x patch/RC reserve.
...
- **Required for v0.1.38 GA**: stable-template project creation, Director planning/checkpoint, at least two redistributable starter packs, real previews, targeted revision, verified native PPTX publication, Studio states, migration routing, bounded Worker/child-process execution, cancellation/cleanup, and rollback.
...
- **Not a GA shortcut**: the legacy bullet writer, an unrendered PPTX, raster-only slides presented as editable, or an unbounded main-process execution path cannot satisfy the stable-template gate. OS-enforced sandbox hardening is deferred to F138 and is not a v0.1.38 gate.
...
- v0.1.38 does not claim an OS security sandbox or containment of a successful native engine exploit. F138 adds OS-enforced filesystem/network/process/resource isolation after the `v0.5.x` product line; its absence does not disable F129 creation, import, rendering, or verification.
...
6. **F138 integration seam** - keep Worker/child-process launch, staged input/output, cancellation, and adapter profiles behind a Space-owned execution interface so the post-v0.5.x OS sandbox can replace the backend without changing Presentation Project manifests or user workflows. F138 is not a v0.1.38 gate.
