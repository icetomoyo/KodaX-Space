...
The user sees three creation choices only:
...
7. **No capability-catalog UI.** Users choose an outcome-oriented creation mode; libraries, converters, and validators stay behind the product boundary.
...
Source and template selection use F130's single `Add material` context surface. After an accepted send, setup asks only for a missing creation choice, source/template, aspect ratio when it cannot be inferred, and a concise audience/purpose confirmation. It appears as a bounded clarification or result-context action, never as a permanent setup mode. Page count, visual tone, citation style, and delivery purpose are inferred and shown in the plan rather than presented as a long form.
...
1. **Results** owns the Presentation Project result row, real slide previews, versions, citations, verification receipts, export actions, and entry into the result-scoped Presentation Studio;
2. **Process** projects `Setup`, `Plan`, `Making`, and `Verify` from the typed F129 lifecycle, including checkpoints, current slide, warnings, recovery, and completion state;
...
Sources and templates remain in F130's context drawer. `Setup`, `Plan`, `Making`, and `Verify` are compact internal stage labels inside Process or Studio, not additional right-rail tabs and not a second persisted lifecycle. The sidebar does not repeat the conversation, advertise internal Skills, list generic capabilities, or show raw tool logs. A row is present only when it helps the user decide, inspect, revise, or recover.
...
│ Setup → Plan → Making → Verify                 │  compact lifecycle; not tabs
├────────────────────────────────────────────────┤
...
| ------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------- |
| `setup`                                           | Process          | Ask the bounded missing decision or cancel setup; material selection opens the shared context drawer          |
| `planning` / `awaiting-design-checkpoint`         | Process          | Show Plan; approve, revise, or switch an enabled creation choice                                              |
| `generating` / `reviewing`                        | Process          | Show Making; inspect current slide, cancel safely, or target a revision                                       |
| `verifying`                                       | Process          | Show Verify and preserve access to the last immutable export in Results                                       |
| Current export passes its target policy           | Results          | Open the new version; download/reveal/version actions and verification receipt remain inspectable             |
| Worker crash, timeout, or cancelled draft         | Process          | Resume from the last valid checkpoint, retry the failed slide, or discard the draft                           |
| Verification failure with a prior verified export | Process          | Explain the failed gate; keep the prior verified version available and badged in Results                      |
| Publishable degraded export                       | Process          | Require explicit acknowledgment before registering that version as the selected Results item                  |
| Rolled-back feature or unsupported future schema  | Results          | Preserve read-only preview/export/receipt access; explain why authoring is unavailable and never downgrade it |
...
| Presentation Studio                                    | Space renderer                                | Setup, plan, page preview, revision, verification, artifact/export projection                                                                            |
...
- F129 does not silently upgrade a legacy artifact into a project. A user may explicitly import an old PPTX as a source/template, which creates a new project and preserves the original delivery unchanged.
...
- Removal means deleting the legacy prompt/tool route, not deleting files or rewriting history. If any retirement gate fails, the feature flag restores the labeled legacy entry without presenting its output as F129 verified.
...
- Register Presentation Project/Studio as a Results adapter, map setup/plan/making/verify into Process, and reuse F130's context drawer for sources/templates.
...
- disclosed grouped/rasterized layers are acceptable for the selected mode;
...
- Results/Process/Files remain the only Partner right-rail tabs; Studio lifecycle stages and material selection do not reintroduce Setup/Making tabs or a permanent Sources rail.
...
- Packaged E2E exercises every feature-flag dependency order, cancellation/kill-tree cleanup, rollback with existing projects, read-only artifact access after rollback, restoration of the labeled legacy route, engine downgrade refusal, and re-enable without manifest rewriting.
