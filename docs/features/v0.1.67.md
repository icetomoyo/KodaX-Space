# v0.1.67 - Partner Presentation Project and Verified Native PPTX Delivery

> Status: Planned
> Feature: F129
> Position: dedicated Partner presentation release; `v0.1.73` is the final 0.1.x patch/RC reserve
...
- No fourth user-facing "hybrid" creation mode and no mixed-authority deck in the v0.1.67 release. An exporter may choose a different native/grouped/raster output representation for fidelity, but it may not change a slide's authoring authority.
...
F129 defines the durable subsystem and its expansion routes, but v0.1.67 does not claim every route before it is proven:

- **Required for v0.1.67 GA**: stable-template project creation, Director planning/checkpoint, at least two redistributable starter packs, real previews, targeted revision, verified native PPTX publication, Studio states, migration routing, bounded Worker/child-process execution, cancellation/cleanup, and rollback.
- **Conditional in v0.1.67**: supplied-template direct fill and creative SVG authoring. Each is enabled only when its dedicated corpus, security, editability, and office-matrix gates pass; otherwise its flag stays off and the roadmap receives a named follow-up target rather than silently shrinking acceptance.
- **Not a GA shortcut**: the legacy bullet writer, an unrendered PPTX, raster-only slides presented as editable, or an unbounded main-process execution path cannot satisfy the stable-template gate. OS-enforced sandbox hardening is deferred to F138 and is not a v0.1.67 gate.
...
- v0.1.67 does not claim an OS security sandbox or containment of a successful native engine exploit. F138 adds OS-enforced filesystem/network/process/resource isolation after the `v0.5.x` product line; its absence does not disable F129 creation, import, rendering, or verification.
...
- F129 ships its own redacted Presentation Project/Studio diagnostics and export receipts in v0.1.67. F094 consumes them in the broader beta diagnostics bundle at v0.1.72; that later integration is not a retroactive v0.1.67 release blocker.
...
6. **F138 integration seam** - keep Worker/child-process launch, staged input/output, cancellation, and adapter profiles behind a Space-owned execution interface so the post-v0.5.x OS sandbox can replace the backend without changing Presentation Project manifests or user workflows. F138 is not a v0.1.67 gate.
