# v0.1.30 - External Agent Orchestration Gateway
...
- [x] full Electron E2E suite passes (58/58), including two dedicated Reference Agent journeys;
...
- KodaX `0.7.67` Reference Executor integration and conformance coverage as the v0.1.30 baseline.
- Capability-gated follow-on integration for A2A, explicit MCP Agent Profile/Tasks, and declarative private HTTP adapters in that order; an adapter is not a v0.1.30 capability until Runtime advertises it and conformance passes.
...
These sections define the adapter contracts Space will consume after the KodaX `0.7.67` neutral substrate. They are not claims that `0.7.67` or the initial v0.1.30 integration already ships the protocol adapters.
...
- [x] `0.7.67` 提供 Reference Executor 并通过核心 conformance；A2A/MCP Tasks/HTTP adapter 未交付时 Runtime capability 不得宣称支持。
...
F115 is deliberately incremental. `v0.1.30` first proves the `0.7.67` neutral substrate with the Reference Executor. Space must not infer A2A/MCP/HTTP support from the existence of Catalog APIs; each protocol UI remains capability-gated until its adapter passes the same lifecycle/security conformance suite.
...
### 14. Follow-on protocol decisions

The Reference baseline is implemented. These choices govern later real-protocol adapter tranches:
...
6. The 0.1.30 Reference baseline is main-owned/embedded only; daemon exposure remains gated until capability/API parity and Host-side factory installation are proven in that mode.
