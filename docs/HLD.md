...
>
> **2026-09-07 当前源码候选**：Space package 为 `0.1.46-alpha.7`，root/Desktop/lockfile 精确锁定 KodaX
> `0.7.96-beta.2`。SDK 包启动门要求 `effectiveConfig:1`、`sandboxRuntime:11`、`runtimeAutoModeGuardrail:5` 与 `sharedSessionSettings:2`；`providerCredentialBroker:2` 由 daemon 准入 requirements 和连接后 Runtime capability
