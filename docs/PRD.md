...
> **2026-08-19 当前正式发布基线**：KodaX Space [`v0.1.43`](https://github.com/icetomoyo/KodaX-Space/releases/tag/v0.1.43) 对齐 npm 正式发布的精确 KodaX `0.7.92`，要求 sandboxRuntime v4、crashOutcomeModel v2、Actor settlement convergence v2、Session-scoped event journal、liveOutputSegments v1 与本地 runtimeExitSettlement v1；v0.1.42 / v0.1.39 保留为历史正式产品基线。
...
> Status: 长期产品方向文档。当前正式发布基线为 KodaX Space 0.1.43（package 0.1.43）/ 精确 Registry KodaX 0.7.92。v0.1.43 在既有 Runtime owner、canonical Actor/Turn、精确 history/live 与 compaction、完整物理请求诊断、F140-F142、可配置 Shell、独立 integration 配置和正式打包门禁基础上，继续收口 SDK 退出结算、sandbox v4 filesystem-effect 收敛、crash-outcome v2 与 live output segments。生命周期支持仍按能力协商，不通过 SemVer 推断；Issue 133 的 macOS/Linux process acceptance/cleanup retry gap 和 F138 完整 OS 隔离继续保持未完成。已交付能力与边界以 [USER_MANUAL.zh-CN.md](USER_MANUAL.zh-CN.md)、[KODAX_CAPABILITY_LEDGER.md](KODAX_CAPABILITY_LEDGER.md) 和 [FEATURE_LIST.md](FEATURE_LIST.md) 为准。
...
| `v0.1.44-v0.1.60` | 维护与稳定化预留；`v0.1.43` 之后无 feature 分配                                                                                                         |
