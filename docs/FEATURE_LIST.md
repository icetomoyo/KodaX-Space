...
> Current source baseline: Space `0.1.46-alpha.1` with exact published prerelease `@kodax-ai/kodax@0.7.96-alpha.5`. Root/Desktop manifests, lockfile, installed bytes, packaged ASAR, and the fully unpacked universal native bundle must resolve the same Registry URL and integrity. Space-managed daemons require explicit capability contracts in addition to the established shared-session safety surface, including local `runtimeExitSettlement:2`, `sandboxRuntime:9`, `runtimeAutoModeGuardrail:5`, `sharedSessionSettings:2`, `providerCredentialBroker:2`, `effectiveConfig:1`, `crashOutcomeModel:2`, `managedRunDurability:1`, `actorSettlementConvergence:2`, `sessionEventJournal:1`, and `conversationHistory:2`; lifecycle support is not inferred from SemVer. Space exposes only Plan, Edits, Auto[LLM], and Full Access; old Rules/engine/timing/envPass state is inert or migrated away. Alpha.5 completes concurrent Windows sandbox admission/self-healing and version-safe idle-daemon replacement. Space consumes exact Registry bytes without patching them. The stable Space v0.1.45 / KodaX 0.7.95 record remains historical and is not rewritten as an alpha release.
