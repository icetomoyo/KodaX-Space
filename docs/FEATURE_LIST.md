...
> Current source baseline: Space `0.1.46-alpha.1` with exact published prerelease `@kodax-ai/kodax@0.7.96-alpha.3`. Root/Desktop manifests, lockfile, installed bytes, packaged ASAR, and the fully unpacked universal native bundle must resolve the same Registry URL and integrity. Space-managed daemons require explicit capability contracts in addition to the established shared-session safety surface, including local `runtimeExitSettlement:2`, `sandboxRuntime:6`, `providerCredentialBroker:2`, `effectiveConfig:1`, `crashOutcomeModel:2`, `managedRunDurability:1`, `actorSettlementConvergence:2`, `sessionEventJournal:1`, and `conversationHistory:2`; lifecycle support is not inferred from SemVer. Space uses scoped leases for manual compaction and lazy multi-Provider routing, and reads effective daemon settings with source provenance. Delivered interrupt `entryId` remains feature-detected per event. Session journal cursors are compared only within the same `(sessionId, journalEpoch)` lineage. KodaX alpha.3 retains the alpha.2 Windows lifecycle PowerShell resolver fix; Space consumes exact Registry bytes without patching them. The stable Space v0.1.45 / KodaX 0.7.95 record remains historical and is not rewritten as an alpha release.
...
| `v0.1.46-alpha.1`  | KodaX 0.7.96-alpha.3 Runtime integration                                                                              | Maintenance / SDK alignment       | Scoped keychain credential leases cover manual compact and lazy multi-Provider Run/Agent/Workflow routing; effective daemon configuration exposes safe source provenance; Space reasoning efforts stay SDK/provider-aware; packaged sandbox smoke validates the v6 native authority path with a bounded lifecycle budget.                                                                 |
