...
- **FEATURE_275, dev source (unreleased)**: canonical pages and the live tail now have separate source storage; the old stabilize/fold/baseline paths are removed and the F144 page contract is exposed. Retirement is per covered input, including partial coverage within one Run. Space source separation and the reviewed fixes are implemented. Strict lifetime replay equivalence remains InProgress: beta.8 preserves confirmed delivery/canonical aliases, while durable exact identity lookup or a complete delivery coverage proof for expired renderer receipts remains missing. Unconfirmed legacy identities are not inferred automatically. See the [implementation correction](features/v0.1.46.md#2026-09-11-修正与票-7-收口dev未发布) and [regression guide](test-guides/FEATURE_275_v0.1.46_TEST_GUIDE.md). No version bump or SDK capability change.
...
| `v0.1.46-alpha.5`  | Foreground transcript convergence guarantee (Issue 206)                                                               | Maintenance                       | The 30s visibility tick and every focus/visibility edge revalidate the current Session's newest canonical page through the generation-fenced, certification-preserving read, so a missed renderer push (occlusion transport drop, reconnect gap) can only delay transcript convergence by one tick instead of requiring a manual reload. Real-runtime probes r7 (fully minimized turn 2 after a 78s gap) and r8 (deterministic missed-notification via main-process push suppression) cover the class. |
| `v0.1.46-alpha.6`  | Ghost query bubble bind-don't-mint hotfix (Issue 207)                                                                 | Maintenance                       | A queued-prompt boundary event that arrives after its delivery already canonicalized can no longer mint a duplicate visible user owner with a self-resolved ordinal and wall-clock sentAt (painted below its own answer; healed only by Ctrl+R). An absent `turnUserOrdinal` is treated as a replay signal: an identical-content owner for the same `turnId` (live or canonical) is bound instead of minting, while explicit daemon ordinals keep minting for legal same-text interrupts. A same-revision idempotence guard makes an identical newest-page read a no-op so the Issue 206 foreground tick stops re-running the fold. Acceptance: 12-case real-lineage repro suite with case J as the symptom proof; the structural rewrite is FEATURE_275 ([features/v0.1.46.md](features/v0.1.46.md)). |
| `v0.1.46-alpha.8` | KodaX 0.7.96-beta.3, compact echo ordering and packaged Windows sandbox repair (Issue 210) | Maintenance / SDK alignment | Manual `/compact` awaits echo and in-flight retry persistence before preflight. Superseded credential connections recover through the existing reconnect path. Physical ASRT resources come from the exact SDK bundle, with WFP probe byte verification and unchanged capability contracts. See [Issue 210 regression guide](test-guides/ISSUE_210_v0.1.46_REGRESSION_GUIDE.md). |
| `v0.1.46-alpha.9`  | KodaX 0.7.96-beta.4 integration, composer submit-clear and compaction summary telemetry | Maintenance / SDK alignment | Exact beta.4 Registry bytes are pinned; manual and automatic compaction share one `compaction.reasoning` summary policy persisted through shared Session settings (Space keeps the SDK default), and successful compactions surface a bounded summary-request count plus durable commit duration through `compact_stats` — the context gauge shows the request count only when map/reduce fans out. The v0.1.45 composer regression is fixed: slash commands clear the input at submit time with a submit-time attachment snapshot, and the compacting spinner stays centered in the context gauge (e2e `compact-composer.spec.ts`). |
| `v0.1.46-alpha.10` | KodaX 0.7.96-beta.6 integration (DeepSeek official Anthropic wire, setup generation 11, prompt-cache wire-truth diagnostics) | Maintenance / SDK alignment | Exact beta.6 Registry bytes are pinned (beta.5 + beta.6 retained; no public capability version moves). The built-in deepseek alias speaks the official api.deepseek.com/anthropic wire with new default model `deepseek-flash` (1M context, 384k max output, native image input) while text-only `deepseek-v4-pro` stays selectable and the `deepseek-v4-flash-vision-exp` experiment leaves the catalog — Space's catalog override protocol follows the SDK truth and becomes `anthropic`. Replayed restored-session histories answer orphaned tool_use calls with the explicit interrupted-tool marker instead of 400-ing on strict endpoints. Windows sandbox setup advances to generation 11 (Issue 333 Codex-compatible profile/SSH ACL exclusions; wire protocol 10 unchanged), and beta.6 makes prompt-cache diagnostics describe the messages actually sent. |
| `v0.1.46-alpha.11` | KodaX 0.7.96-rc.1 integration (permission authority v6, Session Stop frontiers) + FEATURE_275 transcript ownership landing | Maintenance / SDK alignment + transcript rewrite | Exact rc.1 Registry bytes are pinned and startup/daemon/packaging gates now require `runtimeAutoModeGuardrail:6`; embedded runs pass their bound permission mode to SDK text authority so Runtime and direct-SDK text writes agree. rc.1 Session Stop gives durable stop frontiers — one stop cancels the owner's queued frontier while later Runs survive, result-unknown stops keep the original Session/Run/requestId retry identity, and retries are refused after a terminal Run without an owner receipt; managed extension and explicit `!command` Runs execute through the normal Run path, and `/repair-identity` applies only user-verified audited mappings. FEATURE_275 tickets 1-6 land the identity-anchored transcript model: canonical pages plus a transient live tail with explicit input/output ownership, so settling one input can no longer delete sibling inputs in the same Run, and retirement receipts reject journal replay or snapshot sidecars without treating a larger cursor as coverage proof. Packaged boot keeps restart-stable Runtime credentials in an isolated keyring namespace; `identity_repair_invalid` and structured `local_execution` failures preserve readable history without provider-setup suggestions. |
| `v0.1.46-alpha.7`  | KodaX 0.7.96-beta.2 compaction credential fix (Issue 209)                                                             | Maintenance / SDK alignment       | The Issue 199 scoped-credential resolution had shipped with an incomplete daemon-side half: the compaction summarizer resolved its Provider key through the environment-variable path, which returns nothing inside a credential lease scope, so keychain-only Providers failed manual `/compact` and run-internal managed compaction with `<ENV> not set` while the registered broker was never consulted. KodaX 0.7.96-beta.2 routes the summarizer's Provider request through the compaction-purpose lease acquire. Acceptance: protocol-level E2E against a live daemon — the Space-side broker receives exactly one `purpose='compaction'` operation-target request whose `operationId` matches the compact envelope, the keychain key reaches the provider call, and a real local OpenAI-compatible provider completes the compaction (`compacted: true`, 88409 → 39071 tokens) with no skipped/not-set outcome ([KNOWN_ISSUES.md Issue 209](KNOWN_ISSUES.md)). |
| `v0.1.46-alpha.4`  | Settlement certification identity hardening and reader recovery (F274 / Issues 203-205)                               | F274 / Maintenance                | Terminal-run certification anchors on identity evidence — the terminal-scoped newest page must contain the settled Run's turn (`turnId` presence) before that read certifies the Run, with a two-consecutive-identical-`page.revision` structural exit for turns that legitimately persisted nothing, and the Issue 204 empty-durable guard retained as an independent invariant ([ADR-009](ADR/ADR-009-settlement-certification-identity.md)). Issue 203 persists a cross-project Session open; Issue 205 maps the SDK `Session not found` throw to the existing null contract across persisted-session readers through one shared predicate. |
...
| `v0.1.46`          | Settlement certification identity hardening (F274)                                                                    | F274 / Maintenance                | Terminal-run certification anchors on identity evidence — the terminal-scoped newest page must contain the settled Run's turn (`turnId` presence) before that read certifies the Run, with a two-consecutive-identical-`page.revision` structural exit for turns that legitimately persisted nothing, and the Issue 204 empty-durable guard retained as an independent invariant. The cross-domain revision equality fence is rejected by measured contract evidence ([ADR-009](ADR/ADR-009-settlement-certification-identity.md)). |
