...
- **FEATURE_275, dev source (unreleased)**: canonical pages and the live tail now have separate source storage; the old stabilize/fold/baseline paths are removed and the F144 page contract is exposed. Retirement is per covered input, including partial coverage within one Run. Space source separation and the reviewed fixes are implemented. Strict lifetime replay equivalence remains InProgress: the SDK must preserve delivery/canonical aliases and expose durable exact identity lookup or a complete delivery coverage proof for expired renderer receipts. See the [implementation correction](features/v0.1.46.md#2026-09-11-修正与票-7-收口dev未发布) and [regression guide](test-guides/FEATURE_275_v0.1.46_TEST_GUIDE.md). No version bump or SDK capability change.
