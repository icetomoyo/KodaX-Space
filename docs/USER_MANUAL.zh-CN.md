...
> 已发布产品基线：KodaX Space `v0.1.33`（package `0.1.33`）/ KodaX `0.7.77`；
> 当前发布候选：KodaX Space `v0.1.34`（package `0.1.34`）/ npm 正式发布的精确
> KodaX `0.7.78`。
>
...
> 本手册以 `v0.1.34` 发布候选为基线；历史安装包的界面与行为可能不同。
...
### Coder 的 Daemon / Embedded 运行模式（v0.1.34）
