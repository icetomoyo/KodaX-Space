...
- **Official KodaX 0.7.75 Registry synchronization** - Reinstalled the npm-published package, pinned its official Registry URL, SRI (`sha512-a/Xt+LpQ…XfonNMg==`), and tarball SHA256 (`A940D527…D721C72A`), and retained the Coder daemon contract at `contextCompaction:3`, `transcriptPaging:1`, `transcriptSearch:1`, interrupt input v1, Actor control v1, and Auto LLM guardrail v3. All 133 published files match the Registry tarball.
...
- **Published dependency integrity** - The 0.7.75 lock entry matches the official npm Registry package and contains no sibling-checkout path; clean Registry-only installation is reproducible.
...
- **Stale Runtime behavior after upgrade** - Space rejects daemon identities older than 0.7.75 with an actionable restart diagnostic. `/auto-denials` reports the effective Runtime version, classifier model, timeout, speculative window, and bounded classifier timing facts.
...
- **KodaX 0.7.75 Runtime behavior** - The final artifact retains idle-yield user-prompt transcript reporting, crash-recoverable root completion delivery, resident Goal lifecycle contracts, and root/child live projection isolation, while adding audited Windows non-interactive child-process hiding and corrected Sidecar optional-follow-up/budget terminal semantics.
...
- **Windows query console flashes** - KodaX 0.7.75 hides Runtime Worker non-interactive child processes and audits packaged-host query paths while preserving explicit editor, terminal, and PTY interaction.

### Verification

- The installed-package compatibility probe runs against the npm Registry package and requires KodaX 0.7.75, `runtimeAutoModeGuardrail: 3`, `permission:grant-admin`, interrupt input, Actor control v1, compaction v3, transcript paging/search, typed Auto LLM resolution, exact concrete-grant reuse, workspace-edit auto-allow, and prompt-free missing-model rejection.
- Focused Runtime, Sidecar, Windows child-process, permission, attachment, clipboard, schema, provider-catalog, and queue regressions cover the 0.7.75 contracts alongside the F122-F124 source/index/citation/recall suite.
