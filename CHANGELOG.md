...
- **Project HTML File Viewer first-input readiness** - Project previews now defer readiness until authored classic and module scripts finish document initialization, and the iframe remains outside pointer/keyboard interaction until then. A visible control can no longer consume its first click before its module handler exists, and release E2E treats retries as investigation evidence rather than hiding the race.
