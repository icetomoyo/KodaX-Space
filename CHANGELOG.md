...
- **Official KodaX 0.7.74 Registry synchronization** - Reinstalled the npm-published package, pinned its official Registry URL and SRI (`sha512-JgPE6ct9…AfRvGg==`), and retained the Coder daemon contract at `contextCompaction:3`, `transcriptPaging:1`, `transcriptSearch:1`, interrupt input v1, Actor control v1, and Auto LLM guardrail v3. All 133 published files match the Registry tarball byte-for-byte.
...
- **Published dependency integrity** - The 0.7.74 lock entry matches the official npm Registry package and contains no sibling-checkout path; clean Registry-only installation is reproducible.
...
- **Stale Auto LLM behavior after upgrade** - Space rejects daemon identities older than 0.7.74 with an actionable restart diagnostic. `/auto-denials` now reports the effective Runtime version, classifier model, timeout, speculative window, and bounded classifier timing facts.
...
- **Refreshed same-version Runtime behavior** - The installed 0.7.74 artifact now carries idle-yield user-prompt transcript reporting, crash-recoverable root completion delivery, resident Goal lifecycle contracts, hardened interrupt-window closure, and root/child live projection isolation. Space requires the associated exact-history capabilities and records the public failure/ownership semantics without creating duplicate runtime state.
...
- **Windows 11 live-window icon** - Main and standalone Artifact windows now use an explicit runtime `icon.ico` path, and the installer copies the exact multi-size icon beside `app.asar`; package smoke verifies the runtime file plus Setup, Portable, and unpacked application PE resources so the taskbar/Alt+Tab view cannot fall back to a generic blank document icon.
