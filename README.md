...
<p align="center">
  ...
  <img alt="KodaX SDK" src="https://img.shields.io/badge/KodaX_SDK-0.7.82-f0a020?style=flat-square">
...
**`main` uses the exact KodaX 0.7.82 package and negotiates Runtime safety contracts explicitly; it does not infer support from a semantic version.** Coder defaults to the profile-scoped shared daemon with exclusive Actor ownership, canonical bounded Actor/Turn projection, exact history/live reconciliation, durable managed-Run admission, Runtime-owned interrupt finalization, orphan idle exit, complete physical-request usage diagnostics, Auto guardrail v4, resilient integration configuration, and structured tool-sandbox observations. F141 adds a customer-visible Settings → Runtime switch between recommended Daemon mode and an Embedded compatibility fallback. A switch is admitted only after Runtime operations drain and Space proves that no ManagedSession, running/paused Workflow, non-terminal External Agent task, permission/AskUser interaction, queued Coder prompt, daemon work, or other client can be disrupted; owner policy and the versioned preference then transition before Space restarts. Startup reconciles persisted mode with owner policy before Runtime connection. The environment variable is only a one-time migration seed for pre-v3 settings. Partner remains an embedded-inline Space owner, while MCP processes/logs, Workflow library/start/admin, Space Reference Agent execution, and product artifacts remain explicit host-provider boundaries.

KodaX 0.7.82 retains the separate integration declarations and durable managed-Run boundaries introduced in 0.7.80: accepted initial and queued prompts and every completed turn are persisted before their lifecycle event is emitted. Space requires `managedRunDurability:1`, binds the admitted `runId` and streamed `turnId` to its optimistic query, and rejects daemons that cannot make that guarantee. The SDK now defaults an unspecified Auto LLM classifier timeout to 45 seconds and its one retry to 90 seconds; Space forwards only an explicit user timeout. `worker.configuredA2A` is a KodaX CLI Worker-hosted embedded-runtime option, not a Space Settings toggle. The in-app `kodax_manual` composes these Space boundaries with the exact installed SDK's original underlying-capability topics.

KodaX 0.7.82 retains the canonical `entryId` created for every newly delivered interrupt input. Space carries that exact reference through daemon and embedded live events and folds it only when the corresponding conversation entry exposes the same physical ID as its `boundaryId` or one of its proven `auditEntryIds`; disjoint references remain visible, while legacy records without the field keep the existing fail-open compatibility path. v0.1.36 additionally serializes active Session input admission and history revalidation so stale snapshots cannot move a fresh response across queries or Sessions.
...
Released on 2026-08-05 as [`v0.1.36`](https://github.com/icetomoyo/KodaX-Space/releases/tag/v0.1.36), with package version `0.1.36` aligned to the exact npm-published KodaX 0.7.82 package.

| Area                 | Summary                                                                                                                                                                          |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Session admission    | Send, interrupt, after-turn, Session switching, draft restoration, and history revalidation share one bounded admission boundary.                                             |
| History/live identity| Exact `runId`/`turnId`, canonical boundaries, paging cursors, and snapshot watermarks remain bound to the owning Session and query.                                           |
| Renderer recovery    | Stale snapshots and cross-Session events no longer hide activity, duplicate output, reorder turns, or move transcript content between Sessions.                               |
| KodaX baseline       | Root/Desktop manifests and lock views use exact Registry KodaX 0.7.82 bytes and the release gate rejects drift or local tarballs.                                              |
| Documentation        | The Chinese manual, capability ledger, release design, readiness record, and in-app `kodax_manual` describe the same current boundary and explicit limitations.                 |

See [CHANGELOG.md](CHANGELOG.md), the [v0.1.36 design](docs/features/v0.1.36.md), and the [v0.1.36 release record](docs/releases/v0.1.36-release-readiness.md).
...
| [docs/USER_MANUAL.zh-CN.md](docs/USER_MANUAL.zh-CN.md)                                                   | Illustrated Chinese manual for the v0.1.36 release baseline.                             |
...
| [docs/releases/v0.1.36-release-readiness.md](docs/releases/v0.1.36-release-readiness.md)                 | v0.1.36 gates, KodaX 0.7.82 contract, and release evidence.                              |
...
| `v0.1.45`、`v0.1.47-v0.1.49` | Partner Skill workspace, knowledge quality/curation, Presentation Project, and the SDK-gated Memory Agent host.                                                           |
