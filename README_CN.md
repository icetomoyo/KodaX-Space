...
| 范围 | 摘要 |
| --- | --- |
| 最新 SDK | npm `latest` 在发布准备时核验为 `0.7.87`，manifest、lockfile、安装包和 SRI 均精确一致。 |
| Provider recovery | 只清除被替换的临时 assistant/thinking 草稿；工具回执、诊断、manual continuation 和合法重复文本保留。 |
| 文档与手册 | README、用户手册、PRD/HLD、Feature List、能力台账、Known Issues、release 记录、回归指南和 `kodax_manual` 统一到 v0.1.41。 |

详细内容见 [v0.1.41 设计](docs/features/v0.1.41.md)、[发布记录](docs/releases/v0.1.41-release-readiness.md) 和 [Issue 181 回归指南](docs/test-guides/ISSUE_181_v0.1.41_REGRESSION_GUIDE.md)。
...
| External Agents    | KodaX 0.7.87 Runtime 配置的 Coder Agent 使用独占 Actor owner、durable managed Run 和统一 Actor/Turn 任务；Space Reference Agent 保留主窗口管理和 durable Task Dock 干预路径。MCP Tasks 与受治理 HTTP 继续门控。 |
...
| [docs/USER_MANUAL.zh-CN.md](docs/USER_MANUAL.zh-CN.md)                                                   | 面向 v0.1.41 发布基线的图解中文手册。                       |
